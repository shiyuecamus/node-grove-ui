...
export class StepFormApi implements ExtendedStepFormApi {
  ...
  // 获取合并后的所有表单值
  async getMergedValues(): Promise<any> {
    // 先获取当前步骤的值
    await this.getStepValues();

    const mergeValues = this.store.state.mergeValues !== false;

    // 返回合并后的值
    return mergeValues
      ? Object.assign({}, ...Object.values(this.state.formValues))
  ...
  async getStepValues(step?: number): Promise<any> {
    ...
    this.state.formValues[stepIndex] = values;
    return values;
  }
  ...
  async getValues(): Promise<any> {
    ...
    for (let i = 0; i < this.state.total; i++) {
      await this.getStepValues(i);
    }

    return this.getMergedValues();
  }
  ...
  // 提交所有表单
  async submitAllForms(merge = true): Promise<any> {
    // 验证所有表单
    for (let i = 0; i < this.state.total; i++) {
